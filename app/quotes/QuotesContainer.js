const QuotesContainer = ({ text, reference }) => {
  return (
    <div className="mx-auto mt-5 p-4 max-w-2xl bg-white shadow-lg rounded-lg">
      <h3 className="text-lg md:text-xl text-blue-900 text-left">{text}</h3>
      {reference && (
        <p className="text-right text-blue-900 opacity-50 mt-4 text-sm italic">{reference}</p>
      )}
    </div>
  )
}
